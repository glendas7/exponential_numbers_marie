...
public class Assembler implements Serializable {
  /****************************************************************************************************
   * This program assembles code written for the MARIE (Machine Architecture that
   * is Really *
   * Intuitive and Easy) computer as described in *The Essentials of Computer
   * Organization and *
   * Architecture* by Null & Lobur. As with most assemblers, this assembler works
   * in two passes *
   * over the source code: The first pass performs simple translation of mnemonics
   * to their *
   * hex equivalent, e.g., LOAD = 1, and adding whatever symbols that it finds to
   * a symbol table *
   * maintaned by this program. The first pass of the program creates an
   * intermediate workfile *
   * of serialized AssembledCodeLine objects, still containing symbolic names
   * instead of memory *
   * addresses. *
   * *
   * The second pass of the assembler, reads the intermediate workfile and
   * supplies addresses *
   * for the symbols using addresses found in the symbol table. (Or producing an
   * error if the *
   * symbol can't be found.) The output of this phase is written to another
   * workfile. *
   * *
   * The final output of the assembler, the "MARIE EXecutable" file is produced
   * from the second- *
   * pass workfile if there are no errors. A map of symbol addresses from the
   * symbol table is *
   * also produced so that it can be used as an online reference when running the
   * MARIE simulator. *
   * A listing file is produced whether or not the assembly was successful. The
   * output format of *
   * both the assemby listing and symbol table is HTML to facilitate access within
   * the environment *
   * of the MARIE simulator. *
   * *
   * *
   * Implementation Notes: 1. Unlike "real" assemblers, this assembler does not
   * produce binary *
   ...
   * Address literals (instruction operands) must be in hex and in
   * MARIE-addressable range.
   * Numeric literals must be in the range of -32768 to 32767. MARIE uses 16 bits
   * only
  ...
  public static final String[] errorMsgs = {
      ...
      "Clear Instruction is invalid: The instruction was replaced with Load XXXZERO" // 13
  };
  /* -- -- */
  /* -- Instance variables -- */
  /* -- -- */
  ...
  class SymbolEntry {
    /******************************************************************************************
     * Inner class SymbolEntry is the framework for the objects that are used to
     * store and *
     * retrieve values that form the Assembler's symbol table. As these objects are
     * *
     * instantiated, they are placed in a HashTable structure. As references to the
     * symbols *
  ...
  class Instruction implements Serializable {
    /******************************************************************************************
     * Inner class Instruction stores the important components of a Marie machine
     * instruction. *
     ******************************************************************************************/
  ...
  /******************************************************************************************
   * Create searchable instruction set hashtable from a stream of literal values.
   * The *
   * Instruction objects end up in a Hashtable. The format for this hashtable is
   * given *
  ...
  void loadInstructionSet() {
    ...
    instructionSet.put("MULT", new Instruction("MULT", (byte) 10, true));
    ...
    instructionSet.put("DIV", new Instruction("DIV", (byte) 15, true));// P3 Q3
  ...
  String statementLabel(String stmt) {
    /******************************************************************************************
     * Looks for label punctuation in the parameter String. Returns the label if
     * found *
  ...
  boolean tokenIsLiteral(String token) {
    /******************************************************************************************
     * This method determines whether the token passed as a parameter is a valid hex
     * literal. *
     * If a hex literal is used as an address literal (as opposed to a symbolic
     * reference to *
     * an address) the address must begin with a zero, even if it means the literal
     * will be *
     * longer than 3 characters. (This is the only way we can tell the address A
     * from the *
  ...
  int validMarieValue(int number) {
    /******************************************************************************************
     * Used by the literalToInt() method to check the value of the parameter integer
     * with *
     * respect to the 16-bit word size of Marie. Specifically, Java values in the
     * 32768 to *
     * 65535 (absolute value) translate to -32768 -> 0 in MARIE memory. Otherwise,
     * anything *
  ...
  boolean addedToSymbolTable(String symbol) {
    /******************************************************************************************
     * Returns true if argument symbol (along with the line number where it is
     * defined) is *
     * successfully added to the symbol table. If the symbol is already in the
     * symbol table, *
  ...
  String padStr(String s, int size) {
    /******************************************************************************************
     * Adds trailing blanks to pad the string s to the length (size) specified in
     * the *
  ...
  int getOpcode(String stmt) {
    /******************************************************************************************
     * Tries to find the argument stmt in the Hashmap instructionSet. If not found,
     * returns *
     * Java Integer.MIN_VALUE (a number we'd never see in a MARIE instruction set).
     * *
     * Also makes a "special case" check that an ORiGination statement must be the
     * first *
    ...
    if (stmt == "LOAD") { // Try to find stmt value in
  ...
  void setErrorMessage(String msg) {
    /******************************************************************************************
     * Increments the error count, sets the error flag and adds the message string
     * of the *
     * argument to the list (Vector) of errors for the current code line being
     * parsed. *
     ******************************************************************************************/
    errorCount++;
    errorFound = true;
  ...
  AssembledCodeLine parseCodeLine(String inputLine) {
    /******************************************************************************************
     * This method controls extraction of symbols from a single line of source code
     * passed as *
     * a String argument. The return value is an object composed of the extracted
     * tokens *
     * along with a Vector containing any error messages found, and the input line
     * itself. *
     * If the inputLine is a blank line or a comment, the token-related fields in
     * the returned *
     * object are all spaces, and the object is populated only with the source code
     * line *
     * itself. *
     * *
     * Side effects: *
     ...
     * *
     * value) is incremented. Also, if any errors are found (such as a missing
     * token), *
    ...
    if (codeLength > 0) { // Copy noncomment code to working buffer.
      ...
      // System.out.println("ExampleYY " + inputLine.substring(0, codeLength) +
      // " count: " + count);
    ...
    /* -- -- */
    // P3-Q1 Produce error message here
    String aToken = st.nextToken();
    if (inputLine.equals("Load XXZero")) {
    ...
    /* -- -- */
    /*
     * -- Get the operand. If no operand is needed, we would have found out when we
     * got --
     */
    /*
     * -- the opcode. In the process of finding the opcode, the operandReqd flag is
     * set --
     */
  ...
  AssembledCodeLine symbolsToAddresses(AssembledCodeLine codeLine) {
    /******************************************************************************************
     * This is the second pass of the assembler. All we need to do is find the
     * addresses of *
     * the symbols in the assembler program. We either find them in the symbol table
     * or we *
     * don't. *
     ******************************************************************************************/
    ...
    if (codeLine.operand.indexOf((int) '_') == 0) {
      currAddress = getSymbolAddress(codeLine.operand.substring(1, codeLine.operand.length()),
          currAddress);
      if (currAddress != null) {
  ...
  int performFirstPass() {
    /******************************************************************************************
     * This method calls methods to open all of the first-pass files, read the
     * source file, *
     * parse the input, and write the output to the intermediate output file. When
     * complete, *
     ...
     * code. *
     * *
     * When this pass is complete, the "binary" program output is ready for
     * second-pass *
     * processing. *
     ******************************************************************************************/
    ...
    while (!done) { // Loop through source file input.
      try {
        ...
        if (inputLine != null) {
          String trimmed = inputLine.strip();
          // P3-Q1 If 'Clear' then change to Load XXZero
          if (trimmed.equals("Clear")) {
            inputLine = "Load XXZero";
            checkClear = true;
          }
          aCodeLine = parseCodeLine(inputLine);
          objFileOut.writeObject(aCodeLine);
        }
        if ((inputLine == null) && !done && checkClear) {
          // P3-Q1 variable declaration
          inputLine = "XXZero, HEX 0";
          aCodeLine = parseCodeLine(inputLine);
          objFileOut.writeObject(aCodeLine);
          done = true;
        }
        if (inputLine == null && !checkClear)
          done = true;
      } // try
  ...
  int performSecondPass() {
    /******************************************************************************************
     * This method reads the partially assembled code file produced by the first
     * pass *
     * and calls the method that supplies addresses for the symbols in that file.
     * The *
     * The output is <filename>.MEX that is the MARIE executable used by the MARIE *
     * machine simulator. A file, <filename>.map, containing the symbol table is
     * also *
  ...
  void produceFinalOutput() {
    /******************************************************************************************
     * This method produces the final outputs from the MARIE assembler, using the
     * ".MO2" *
     * file as input. An assembly listing, <filename>.LST is always produced by this
     * step. *
     * If assembly was error-free, a ".MEX" (MARIE EXecutable) file is produced
     * along with *
  ...
  void dumpSymbolTable() throws IOException {
    /******************************************************************************************
     * Place the contents of the symbol table, including line number where defined
     * and line *
     * numbers where referenced, on the assembly listing. We attempt to make a
     * nicely- *
     * formatted table, padding out all symbol names to match the length of the
     * longest name. *
     * The "symbol name" column will be a minumum of 6 characters and a maximum of *
     * MAX_SYMBOL_PRINT_LEN. Symbols longer than MAX_SYMBOL_PRINT_LEN will be
     * truncated for *
     * printing purposes, but the entire symbol name is significant (up to the
     * limits imposed *
     * by the Java language). *
     * If assembly was successful, we also write symbol table entries to a plain
     * text *
     * "mapfile" for later reference (by the progammer) while the MARIE program is
     * executing. *
     ******************************************************************************************/
    SymbolEntry se;
  ...
  public static int assembleFile(String fileName) {
    /******************************************************************************************
     * This method is the mainline for the MARIE assembler. It expects to be passed
     * the *
     * name of a MARIE assembly code file, <filename>, that will be opened as
     * <filename>.MAS. *
     * Ultimately, a <filename>.LST, <filename>.MAP and <filename>.MEX files will be
     * created *
  ...
  void openFirstPassFiles() {
     ...
     * The filename, sourceFileName (class field), has ".MAS" appended to it prior
     * to any *
     * sourcefile open attempts. An intermediate file <sourceFileName>.MO1 is
     * created to *
     * hold assembled code "objects" used for input by the second pass of the
     * assembler. *
     ******************************************************************************************/
  ...
  void openSecondPassFiles() {
     ...
     * We expect to see an ".M01" file that was created by the first pass. We will
     * write the *
     * output of this assembler phase to an ".MEX" file, which consists of assembled
     * MARIE *
     * code lines. *
     ******************************************************************************************/
    try { // Try to open the input.
  ...
  void openFinalFiles() {
    /******************************************************************************************
     * The "final" assembler phase writes the output to a text listing file and
     * creating the *
     * "binary" output. Note the objectFile workfile from the second pass is our
     * input. *
     ******************************************************************************************/
  ...
  void closeFinalFiles() {
    /******************************************************************************************
     * Closes files opened in the final pass, deleting the "binary" objectFile if
     * assembly *
     * was unsuccessful. *
     ******************************************************************************************/
  ...
  public static void main(String args[]) {
    /******************************************************************************************
     * This main method runs the MARIE assembler in standalone console mode by
     * providing a *
